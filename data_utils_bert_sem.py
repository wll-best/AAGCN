...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        all_data = []
        for i in range(0, len(lines), 3):
            text_left, _, text_right = [s.lower().strip() for s in lines[i].partition("$T$")]#原数据集里没有这个？
            text = lines[i].lower().strip()
            aspect = lines[i + 1].lower().strip()#方面类。原来的任务是根据“方面类+句子”，推测极性的
            ...
            entity_graph = np.pad(idx2graph[graph_id], \
...
class IADataset(Dataset):
    def __init__(self, fname, tokenizer):
        fin = open(fname, 'r', encoding='utf-8', newline='\n', errors='ignore')#raw.tokenized文件
        lines = fin.readlines()
        fin.close()
        fin = open(fname+'.graph_entity', 'rb')#raw.tokenized.graph_entity文件
        idx2graph = pickle.load(fin)
        fin.close()
        # if '15' in fname or '16' in fname:#不要
        #     fin = open(fname + '.graph_attribute', 'rb')
        #     idx2graph_a = pickle.load(fin)
        #     fin.close()
        graph_id = 0
