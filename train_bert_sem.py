...
#前两个用不到,第三个不改，最后一个ABSA改成IA
from data_utils_bert_sem import Tokenizer4Bert, IADataset
...
class Instructor:
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            '''
        self.trainset = IADataset(opt.dataset_file['train'], tokenizer)#ABSADataset
        self.testset = IADataset(opt.dataset_file['test'], tokenizer)#ABSADataset
        # 验证集用自己的
        self.valset = IADataset(opt.dataset_file['dev'], tokenizer)#ABSADataset
        '''
...
def main():
    ...
    parser.add_argument('--dataset', default='sem', type=str, help='twitter, restaurant, laptop')#新增sem
    parser.add_argument('--optimizer', default='adam', type=str)
    parser.add_argument('--initializer', default='xavier_uniform_', type=str)
    parser.add_argument('--lr', default=2e-5, type=float, help='try 5e-5, 2e-5 for BERT, 1e-3 for others')#2e-5
    ...
    dataset_files = {
        ...
        'sem_all_sen':{
            'train':'./dataset/senticnet/ntrain.raw.tokenized',
            'dev': './dataset/senticnet/ndev.raw.tokenized',#验证集用自己的
            'test': './dataset/senticnet/ntest.raw.tokenized'
