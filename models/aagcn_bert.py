...
class GraphConvolution(nn.Module):
    ...
    def forward(self, text, adj):
        ...
        # print("denom")
        # print(type(denom),denom.size())#<class 'torch.Tensor'> torch.Size([16, 85, 1])
        # print('----------')
        # print("adj")
        # print(type(adj), adj.size())#<class 'torch.Tensor'> torch.Size([16, 85, 85])
        # print('----------')
        # print("hidden")
        # print(hidden.float(),type(hidden.float()), hidden.float().size())#<class 'torch.Tensor'> torch.Size([16, 85, 768])
        output = torch.matmul(adj, hidden.float()) / denom
        # print('----------')
        # print("output")
        # print(type(output), output.size())#<class 'torch.Tensor'> torch.Size([16, 85, 768])
        # print('exit')
        # exit()
        if self.bias is not None:
            #print('biasnot')
            #print((output + self.bias).size())#torch.Size([16, 85, 768])
...
class AAGCN_BERT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print('x1')
        # print(x.size())#torch.Size([16, 85, 768])
        # print(a_adj.size())#rest15的torch.Size([16, 85, 85])----#rest14的torch.Size([16])
        #x = F.relu(self.gc2(x, a_adj))#
        # print('x2')
        # print(x.size())#torch.Size([16, 85, 768]) 16最后一批次可能不足16--batch_size,85--max_seq_len,768--bert_dim
